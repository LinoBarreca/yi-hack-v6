# Run without an SD card

## Why did I change this

Most camera firmwares need the SD card plugged in at all times to work. But SD
cards wear out, get corrupted, and are one more thing that can fail. We wanted the
...
2. Open the camera's web address in your browser. If the full interface isn't
   available, you'll get the **recovery page** instead.
3. On the recovery page you can:
   ...
   - point the camera at a **network share** to load the full firmware from the
     network — see
     [Load the firmware from a network share](load-firmware-from-network-share.md),
   - **reboot** the camera.
...
> interface and local recording. Running without the SD card is your choice.
