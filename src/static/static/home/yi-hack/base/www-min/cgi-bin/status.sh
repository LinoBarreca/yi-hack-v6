...
CF=no; mount | grep -q ' /tmp/cifs ' && CF=yes
# busybox here has no 'readlink'; derive the symlink target from 'ls -ld'
if [ -L /home/yi-hack/extra ]; then EX=$(ls -ld /home/yi-hack/extra 2>/dev/null | sed 's/.* -> //'); else EX=none; fi
